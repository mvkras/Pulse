...
//----------------------------------------------------------------------------------------------------------------
    ...
    /* Маски ввода */
      ...
      /* ---------------ВЗАИМОДЕЙСТВИЕ С СЕРВЕРОМ ЛОКАЛЬНЫМ------------------- */
      $('form').submit(function(e){  /* При нажатии на кнопко подтвертидть, происходит действие e- event */
        e.PriventDefault();  /* перезагрузки браузера не будет происходить e.PriventDefault позволяет отменить стандартное поведение браузера */
        $.ajax({
          type: "POST",
          url: "mailer/smart.php", /* Куда будем отправлять наш запрос */
          data: $(this).serialize()  /* Те данные которые хотим отправить на сервер */
        }).done(function(){  /* Когда мы выполнили данную операцию, выполним еще и это действие */
          $(this).find("input").val(""); /* После отправки формы, отчистим все инпуты */
          $('#consultation, #order').fadeOut(); /* После отправки, модальные окна скрываются */
          $('overlay, #thanks').fadeIn('slow');

          $('form').trigger('reset'); /* Запускаем, что все формы должны сброситься */
        });
        return false;
      });

      /* СКРОЛЛ СТРЕЛКИ ВВЕРХ PAGE UP */
   ...
   new WOW().init(); /* Прописываем анимацию для текста ОТЗЫВЫ КЛИЕНТОВ, СОВМЕСТНО С ФАЙЛОМ ANIMATE.CSS */
});  
